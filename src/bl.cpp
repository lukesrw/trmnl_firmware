                            ...
                            case SF_REWIND:
                                {
                                    ...
                                    if (action.equals("rewind")) {
                                        ...
                                        bool result = fileSystemReadFile("/last.bmp", buffer, sizeof(buffer));
                                        if (result) {
                            ...
                            case SF_SEND_TO_ME:
                                {
                                    ...
                                    if (action.equals("send_to_me")) {
                                        ...
                                        bool result = fileSystemReadFile("/current.bmp", buffer, sizeof(buffer));
                                        if (result) {
                                            bool image_reverse = false;
                                            bmp_err_e res = parseBMPHeader(buffer, image_reverse);
                                            String error = "";
                ...
                case BMP_NO_ERR:
                    {
                        ...
                        if (fileSystemExists("/last.bmp") && fileSystemExists("/current.bmp")) {
                            Log.info("%s [%d]: Last and current exist!\r\n", __FILE__, __LINE__);
                            if (fileSystemDeleteFile("/last.bmp") && fileSystemRenameFile("/current.bmp", "/last.bmp")) {
...
static void log_POST(char* log_buffer, size_t size) {
    ...
    if (!result) {
        ...
        // log not send
        storeLog(log_buffer, size, preferences);
    }
}
...
static void checkLogNotes(void) {
    ...
    if (log.length() > 0) {
        ...
        result = submitLogToApi(input);
    } else {
        Log.info("%s [%d]: no needed to send the log\r\n", __FILE__, __LINE__);
    }
    if (result == true) {
        removeStoredLogs(preferences);
    }
}

static void writeImageToFile(const char* name, uint8_t* in_buffer, size_t size) {
    size_t res = fileSystemWriteFile(name, in_buffer, size);
    if (res != size) {
...
static uint8_t* storedLogoOrDefault(void) {
    if (fileSystemReadFile("/logo.bmp", buffer, sizeof(buffer))) {
        return buffer;
    }
